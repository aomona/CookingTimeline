...
function cookingTemplate(recipe) {
  const title = (recipe && (recipe.title || recipe.id)) || '';
  const total = (recipe?.steps || []).reduce((max, s) => Math.max(max, s.timeline?.end || 0), 0);

  return `
    <div class="max-w-3xl mx-auto">
      <nav class="mb-6">
        <a href="#${recipe?.id || ''}" class="inline-flex items-center px-4 py-2 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow text-gray-700 hover:text-blue-600 border border-gray-200">
      ...
      <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-2xl font-bold text-gray-800">${title} – 調理中</h2>
...
function notify(msg) {
  // try system notification first, fallback to toast
...
export async function renderCookingById(id) {
  ...
  container.innerHTML = cookingTemplate(recipe);
  showSection('cooking');

  const steps = (recipe?.steps || []).filter(s => s?.timeline && typeof s.timeline.start === 'number' && typeof s.timeline.end === 'number');
  const total = steps.reduce((max, s) => Math.max(max, s.timeline.end), 0);
  ...
  function renderActive(t) {
    // Active steps: start <= t < end, sort by start desc (later first)
    const active = steps.filter(s => s.timeline.start <= t && t < s.timeline.end)
    ...
    // Helper: resolve materials used by a step
    function materialsForStep(step) {
      if (!step?.uses || !Array.isArray(step.uses) || step.uses.length === 0) return [];
      const mats = recipe?.materials || [];
      const byId = new Map(mats.map(m => [String(m.id), m]));
      const list = [];
      for (const mid of step.uses) {
        const item = byId.get(String(mid));
    ...
    elActive.innerHTML = active.map((s, idx) => {
      ...
      const mats = materialsForStep(s);
      const matsHtml = mats.length
        ? `<div class="mt-2 text-xs text-gray-700"><span class="font-medium">使用する材料:</span> ${mats.map(m => `${m.name}${m.amount ? `（${m.amount}）` : ''}`).join(' ・ ')}</div>`
        : '';
      return `
  ...
  // ===== Live timeline (similar to recipe view) =====
  const stepColors = [
  ...
  function tick() {
    ...
    // Notify for steps that just ended
    const justEnded = steps.filter(s => lastT < s.timeline.end && s.timeline.end <= t && !endedNotified.has(s.id));
    if (justEnded.length) {
      justEnded.forEach(s => {
    ...
    // update live timeline pointer and highlight active rows
  ...
  function changeSpeed(newSpeed) {
    const t = currentT(); // capture current timeline minute
    speed = newSpeed;
    msPerMin = 60000 / speed;
    // Keep continuity when running
    if (!paused) {
